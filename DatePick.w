...
/*------------------------------------------------------------------------
  ...
  VDATE = TODAY - 150.
  run SHARE/DATEPICK.W (INPUT-OUTPUT VDATE).
  MESSAGE VDATE VIEW-AS ALERT-BOX INFO BUTTONS OK.
------------------------------------------------------------------------*/
...
&Scoped-Define ENABLED-OBJECTS btnHome reBorderIn reBorderOut reSelectedDay ~
btnNextMonth btnNextYear btnPrevMonth btnPrevYear 
...
DEFINE BUTTON btnHome  NO-FOCUS FLAT-BUTTON
     LABEL "Today" 
     SIZE-PIXELS 125 BY 19
     FONT 6.
...
DEFINE VARIABLE fiDayName-1 AS CHARACTER FORMAT "XX":U INITIAL "Pt" 
      VIEW-AS TEXT 
     SIZE-PIXELS 13 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-2 AS CHARACTER FORMAT "XX":U INITIAL "S" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-3 AS CHARACTER FORMAT "XX":U INITIAL "Ç" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-4 AS CHARACTER FORMAT "XX":U INITIAL "P" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-5 AS CHARACTER FORMAT "XX":U INITIAL "C" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-6 AS CHARACTER FORMAT "XX":U INITIAL "Ct" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.

DEFINE VARIABLE fiDayName-7 AS CHARACTER FORMAT "XX":U INITIAL "P" 
      VIEW-AS TEXT 
     SIZE-PIXELS 14 BY 12
     FGCOLOR 1  NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btnHome reBorderIn reBorderOut reSelectedDay btnNextMonth btnNextYear 
...
/*------------------------------------------------------------------------
  ...
  ASSIGN 
    gcDayNames   = 'Pazar,Pazartesi,Salı,Çarşamba,Perşembe,Cuma,Cumartesi'
    gcMonthNames = 'Ocak,Şubat,Mart,Nisan,Mayıs,Haziran,Temmuz,Ağustos,Eylül,Ekim,Kasım,Aralık'
    .

  DO WITH FRAME {&FRAME-NAME}:

    /* Get fonts */
    FRAME {&FRAME-NAME}:FONT = 4.  /* getFont('Default'). */
